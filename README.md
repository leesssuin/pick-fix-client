...
처음에는 비율을 설정하는데 원하는 자리에 배치하는 것이 힘들었습니다.<br>
...
`useRecoilValueLoadable`이 그 메서드고, 이 메서드를 이용해 바로 서버와 통신할 수 있는데, 여기서 고민되었던 부분이 있었습니다.<br>
...
`useRecoilValueLoadable` 을 사용하여 lading중, data를 받아왔을 떄의 비동기 흐름 로직 작업 진행중입니다.(22.05.26 기준)
...
## **AWS S3 에러 해결**
웹과 달리 react-native에서는 파일을 formData로 만들때 객체가 아닌 배열로 만들어져서 서버로 전송까지 했으나 multer에서 인식이 안되어, formData를 객체로 만들어주었습니다. 그러나 multer가 그래도 인식을 하지 못해 아직 수정중입니다.(22.05.26 기준)
<br>
<br>
